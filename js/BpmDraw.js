/**
 * BpmDraw constructor executed on page load.
 */
BpmDraw = function(chromeless, themes, model, graph, editable)
{
	...
	{
		return this.filename || bpmResources.get('drawing', [BpmDraw.pageCounter]) + '.xml';
	};
	...
	{
		this.status = value;
		this.fireEvent(new bpmEventObject('statusChanged'));
	};
	...
	this.graph.getModel().addListener(bpmEvent.CHANGE, bpmUtils.bind(this, function()
	{
...
(function()
{
	try
	{
		...
		while (op.opener != null && typeof op.opener.BpmDraw !== 'undefined' &&
			!isNaN(op.opener.BpmDraw.pageCounter) &&	
		...
		if (op != null)
		{
			op.BpmDraw.pageCounter++;
			BpmDraw.pageCounter = op.BpmDraw.pageCounter;
		}
	}
	catch (e)
	{
 ...
 */
BpmDraw.prototype.cancelFirst = true;

/**
 * Specifies if the editor is enabled. Default is true.
 */
BpmDraw.prototype.enabled = true;

/**
 * Contains the name which was used for the last save. Default value is null.
 */
BpmDraw.prototype.filename = null;

/**
 * Contains the current modified state of the diagram. This is false for
 * new diagrams and after the diagram was saved.
 */
BpmDraw.prototype.modified = false;

/**
 * Specifies if the diagram should be saved automatically if possible. Default
 * is true.
 */
BpmDraw.prototype.autosave = true;

/**
 * Specifies the top spacing for the initial page view. Default is 0.
 */
BpmDraw.prototype.initialTopSpacing = 0;

/**
 * Specifies the app name. Default is document.title.
 */
BpmDraw.prototype.appName = document.title;

/**
 * 
 */
BpmDraw.prototype.editBlankUrl = window.location.protocol + '//' + window.location.host + '/';

/**
 * Default value for the graph container overflow style.
 */
BpmDraw.prototype.defaultGraphOverflow = 'hidden';

/**
 * Initializes the environment.
 */
BpmDraw.prototype.init = function() { };

/**
 * Sets the XML node for the current diagram.
 */
BpmDraw.prototype.isChromelessView = function()
{
 ...
 * Sets the XML node for the current diagram.
 */
BpmDraw.prototype.setAutosave = function(value)
{
	this.autosave = value;
	this.fireEvent(new bpmEventObject('autosaveChanged'));
};

/**
 * 
 */
BpmDraw.prototype.getEditBlankUrl = function(params)
{
 ...
 * 
 */
BpmDraw.prototype.editAsNew = function(xml, title)
{
	...
	else
	{
		if (typeof window.postMessage !== 'undefined' && (document.documentMode == null || document.documentMode >= 10))
		{
			...
			{
				bpmEvent.addListener(window, 'message', bpmUtils.bind(this, function(evt)
				{
 ...
 * Sets the XML node for the current diagram.
 */
BpmDraw.prototype.createGraph = function(themes, model)
{
	var graph = new Draw(null, model, null, null, themes);
 ...
 * Sets the XML node for the current diagram.
 */
BpmDraw.prototype.resetGraph = function()
{
	...
	this.graph.pageScale = bpmGraph.prototype.pageScale;
	this.graph.pageBpmScheme = bpmGraph.prototype.pageBpmScheme;
 ...
 * Sets the XML node for the current diagram.
 */
BpmDraw.prototype.readGraphState = function(node)
{
	this.graph.gridEnabled = node.getAttribute('grid') != '0' && (!this.isChromelessView() || urlParams['grid'] == '1');
	this.graph.gridSize = parseFloat(node.getAttribute('gridSize')) || bpmGraph.prototype.gridSize;
	...
	{
		this.graph.pageBpmScheme = new bpmRectangle(0, 0, pw, ph);
	}
 ...
 * Sets the XML node for the current diagram.
 */
BpmDraw.prototype.setGraphXml = function(node)
{
	if (node != null)
	{
		var dec = new bpmCodec(node.ownerDocument);
	
		if (node.nodeName == 'bpmGraphModel')
		{
			...
			this.fireEvent(new bpmEventObject('resetGraphView'));
		}
		else if (node.nodeName == 'root')
		{
			this.resetGraph();
			
			// Workaround for invalid XML output in Firefox 20 due to bug in bpmUtils.getXml
			var wrapper = dec.document.createElement('bpmGraphModel');
		...
		{
			throw { 
			    message: bpmResources.get('cannotOpenFile'), 
			    node: node,
 ...
 */
BpmDraw.prototype.getGraphXml = function(ignoreSelection)
{
	ignoreSelection = (ignoreSelection != null) ? ignoreSelection : true;
	var node = null;
	if (ignoreSelection)
	{
		var enc = new bpmCodec(bpmUtils.createXmlDocument());
		node = enc.encode(this.graph.getModel());
	}
	else
	{
		node = this.graph.encodeCells(bpmUtils.sortCells(this.graph.model.getTopmostCells(
	...
	node.setAttribute('pageWidth', this.graph.pageBpmScheme.width);
	node.setAttribute('pageHeight', this.graph.pageBpmScheme.height);
 ...
 */
BpmDraw.prototype.updateGraphComponents = function()
{
	...
	if (graph.container != null)
	{
		...
		this.fireEvent(new bpmEventObject('updateGraphComponents'));
	}
};
 ...
 */
BpmDraw.prototype.setModified = function(value)
{
	this.modified = value;
};
 ...
 * Sets the filename.
 */
BpmDraw.prototype.setFilename = function(value)
{
	this.filename = value;
};
 ...
 */
BpmDraw.prototype.createUndoManager = function()
{
	var graph = this.graph;
	var undoMgr = new bpmUndoManager();
	...
	undoMgr.addListener(bpmEvent.UNDO, undoHandler);
	undoMgr.addListener(bpmEvent.REDO, undoHandler);
 ...
 */
BpmDraw.prototype.initStencilRegistry = function() { };

/**
 * Creates and returns a new undo manager.
 */
BpmDraw.prototype.destroy = function()
{
	if (this.graph != null)
	{
		this.graph.destroy();
 ...
 */
function BpmModal(editorUi, elt, w, h, modal, closable, onClose, noScroll, transparent, onResize, ignoreBgClick)
{
	var dx = 0;
	
	if (bpmCore.IS_VML && (document.documentMode == null || document.documentMode < 8))
	{
		// Adds padding as a workaround for box model in older IE versions
		// This needs to match the total padding of geBpmModal in CSS
		dx = 80;
	}
	...
	if (this.bg == null)
	{
		...
		this.bg.style.background = BpmModal.backdropColor;
		this.bg.style.height = dh + 'px';
		this.bg.style.right = '0px';
		this.bg.style.zIndex = this.zIndex - 2;
		
		bpmUtils.setOpacity(this.bg, this.bgOpacity);
		
		if (bpmCore.IS_QUIRKS)
		{
			new bpmDivResizer(this.bg);
		}
	}
	
	var origin = bpmUtils.getDocumentScrollOrigin(document);
	...
	var div = editorUi.createDiv(transparent? 'geTransBpmModal' : 'geBpmModal');
	...
	if (closable)
	{
		...
		img.setAttribute('title', bpmResources.get('close'));
		img.className = 'geBpmModalClose';
		img.style.top = (top + 14) + 'px';
		img.style.left = (left + w + 38 - dx) + 'px';
		img.style.zIndex = this.zIndex;
		
		bpmEvent.addListener(img, 'click', bpmUtils.bind(this, function()
		{
			editorUi.hideBpmModal(true);
		}));
		...
		if (!ignoreBgClick)
		{
			bpmEvent.addGestureListeners(this.bg, null, null, bpmUtils.bind(this, function(evt)
			{
				editorUi.hideBpmModal(true);
			}));
		}
	...
	this.resizeListener = bpmUtils.bind(this, function()
	{
		if (onResize != null)
		{
			var newWH = onResize();
	...
	bpmEvent.addListener(window, 'resize', this.resizeListener);

	this.onBpmModalClose = onClose;
	this.container = div;
	
	editorUi.editor.fireEvent(new bpmEventObject('showBpmModal'));
};

/**
 * 
 */
BpmModal.backdropColor = 'white';

/**
 * 
 */
BpmModal.prototype.zIndex = bpmPopupMenu.prototype.zIndex - 1;

/**
 * 
 */
BpmModal.prototype.noColorImage = (!bpmCore.IS_SVG) ? IMAGE_PATH + '/nocolor.png' : 'data:image/png;base64,';

/**
 * 
 */
BpmModal.prototype.closeImage = (!bpmCore.IS_SVG) ? IMAGE_PATH + '/close.png' : 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAkAAAAJAQMAAADaX5RTAAAABlBMVEV7mr3///+wksspAAAAAnRSTlP/AOW3MEoAAAAdSURBVAgdY9jXwCDDwNDRwHCwgeExmASygSL7GgB12QiqNHZZIwAAAABJRU5ErkJggg==';

/**
 * 
 */
BpmModal.prototype.clearImage = (!bpmCore.IS_SVG) ? IMAGE_PATH + '/clear.gif' : 'data:image/gif;base64,';

/**
 * 
 */
BpmModal.prototype.lockedImage = (!bpmCore.IS_SVG) ? IMAGE_PATH + '/locked.png' : 'data:image/png;base64,';

/**
 * 
 */
BpmModal.prototype.unlockedImage = (!bpmCore.IS_SVG) ? IMAGE_PATH + '/unlocked.png' : 'data:image/png;base64,';

/**
 * Removes the dialog from the DOM.
 */
BpmModal.prototype.bgOpacity = 80;

/**
 * Removes the dialog from the DOM.
 */
BpmModal.prototype.getPosition = function(left, top)
{
	return new bpmPoint(left, top);
};

/**
 * Removes the dialog from the DOM.
 */
BpmModal.prototype.close = function(cancel, isEsc)
{
	if (this.onBpmModalClose != null)
	{
		if (this.onBpmModalClose(cancel, isEsc) == false)
		{
			return false;
		}
		
		this.onBpmModalClose = null;
	}
	...
	bpmEvent.removeListener(window, 'resize', this.resizeListener);
 ...
 * 
 */
var ErrorBpmModal = function(editorUi, title, message, buttonText, fn, retry, buttonText2, fn2, hide, buttonText3, fn3)
{
	...
	if (retry != null)
	{
		var retryBtn = bpmUtils.button(bpmResources.get('tryAgain'), function()
		{
	...
	if (buttonText3 != null)
	{
		var btn3 = bpmUtils.button(buttonText3, function()
		{
			if (fn3 != null)
			{
				fn3();
	...
	var btn = bpmUtils.button(buttonText, function()
	{
		if (hide)
		{
			editorUi.hideBpmModal();
	...
	if (buttonText2 != null)
	{
		var mainBtn = bpmUtils.button(buttonText2, function()
		{
			if (hide)
			{
				editorUi.hideBpmModal();
 ...
 */
var PrintBpmModal = function(editorUi, title)
{
 ...
 * Constructs a new print dialog.
 */
PrintBpmModal.prototype.create = function(editorUi)
{
	...
	bpmUtils.write(span, ' ' + bpmResources.get('fitPage'));
	td.appendChild(span);
	
	bpmEvent.addListener(span, 'click', function(evt)
	{
	...
	bpmEvent.addListener(onePageCheckBox, 'change', function()
	{
		pageCountCheckBox.checked = !onePageCheckBox.checked;
	});
	...
	var span = document.createElement('span');
	bpmUtils.write(span, ' ' + bpmResources.get('posterPrint') + ':');
	td.appendChild(span);
	...
	bpmUtils.write(td, ' ' + bpmResources.get('pages') + ' (max)');
	row.appendChild(td);
	tbody.appendChild(row);

	bpmEvent.addListener(pageCountCheckBox, 'change', function()
	{
	...
	td = document.createElement('td');
	bpmUtils.write(td, bpmResources.get('pageScale') + ':');
	row.appendChild(td);
	...
	function preview(print)
	{
		...
		var pf = graph.pageBpmScheme || bpmConstants.PAGE_FORMAT_A4_PORTRAIT;
		...
		if (autoOrigin)
		{
			...
			if (!isNaN(pageCount))
			{
				scale = bpmUtils.getScaleForPageCount(pageCount, graph, pf);
			}
		}
		...
		// Applies print scale
		pf = bpmRectangle.fromRectangle(pf);
		...
		var preview = PrintBpmModal.createPrintPreview(graph, scale, pf, border, x0, y0, autoOrigin);
		preview.open();
	
		if (print)
		{
			PrintBpmModal.printPreview(preview);
		}
	};
	...
	if (PrintBpmModal.previewEnabled)
	{
		var previewBtn = bpmUtils.button(bpmResources.get('preview'), function()
		{
			editorUi.hideBpmModal();
			preview(false);
		});
	...
	var printBtn = bpmUtils.button(bpmResources.get((!PrintBpmModal.previewEnabled) ? 'ok' : 'print'), function()
	{
		editorUi.hideBpmModal();
		preview(true);
	});
 ...
 * Constructs a new print dialog.
 */
PrintBpmModal.printPreview = function(preview)
{
	try
	{
		if (preview.wnd != null)
 ...
 * Constructs a new print dialog.
 */
PrintBpmModal.createPrintPreview = function(graph, scale, pf, border, x0, y0, autoOrigin)
{
	var preview = new bpmPrintPreview(graph, scale, pf, border, x0, y0);
	preview.title = bpmResources.get('preview');
	preview.printBackgroundImage = true;
	preview.autoOrigin = autoOrigin;
	var bg = graph.background;
	
	if (bg == null || bg == '' || bg == bpmConstants.NONE)
	{
		bg = '#ffffff';
	}
 ...
 */
PrintBpmModal.previewEnabled = true;

/**
 * Constructs a new page setup dialog.
 */
var PageSetupBpmModal = function(editorUi)
{
	var graph = editorUi.editor.graph;
	var row, td;
	...
	td.style.fontSize = '10pt';
	bpmUtils.write(td, bpmResources.get('paperSize') + ':');
	...
	var accessor = PageSetupBpmModal.addPageBpmSchemePanel(td, 'pagesetupdialog', graph.pageBpmScheme);
	...
	{
		if (newBackgroundColor == null || newBackgroundColor == bpmConstants.NONE)
		{
			backgroundButton.style.backgroundColor = '';
			backgroundButton.style.backgroundImage = 'url(\'' + BpmModal.prototype.noColorImage + '\')';
		}
		else
		{
	...
	bpmEvent.addListener(backgroundButton, 'click', function(evt)
	{
	...
	bpmUtils.write(td, bpmResources.get('gridSize') + ':');
	...
	bpmEvent.addListener(gridSizeInput, 'change', function()
	{
	...
	bpmUtils.write(td, bpmResources.get('image') + ':');
	...
	function updateBackgroundImage()
	{
		if (newBackgroundImage == null)
		{
			...
			changeImageLink.innerHTML = bpmResources.get('change') + '...';
		}
		else
		{
	...
	bpmEvent.addListener(changeImageLink, 'click', function(evt)
	{
		editorUi.showBackgroundImageBpmModal(function(image)
		{
	...
	var applyBtn = bpmUtils.button(bpmResources.get('apply'), function()
	{
		...
		if (graph.pageBpmScheme.width != change.previousBpmScheme.width ||
			graph.pageBpmScheme.height != change.previousBpmScheme.height ||
 ...
 * 
 */
PageSetupBpmModal.addPageBpmSchemePanel = function(div, namePostfix, pageBpmScheme, pageBpmSchemeListener)
{
	...
	bpmUtils.write(portraitSpan, bpmResources.get('portrait'));
	...
	bpmUtils.write(landscapeSpan, bpmResources.get('landscape'));
	...
	bpmUtils.write(customDiv, ' in x ');
	
	var heightInput = document.createElement('input');
	heightInput.setAttribute('size', '7');
	heightInput.style.textAlign = 'right';
	customDiv.appendChild(heightInput);
	bpmUtils.write(customDiv, ' in');

	formatDiv.style.display = 'none';
	customDiv.style.display = 'none';
	
	var pf = new Object();
	var formats = PageSetupBpmModal.getBpmSchemes();
	...
	for (var i = 0; i < formats.length; i++)
	{
		...
		bpmUtils.write(paperSizeOption, f.title);
		...
		if (force || (widthInput != document.activeElement && heightInput != document.activeElement))
		{
			...
			for (var i = 0; i < formats.length; i++)
			{
				...
				else if (f.format != null)
				{
					...
					if (f.key == 'a4')
					{
						if (pageBpmScheme.width == 826)
						{
							pageBpmScheme = bpmRectangle.fromRectangle(pageBpmScheme);
							pageBpmScheme.width = 827;
						}
						else if (pageBpmScheme.height == 826)
						{
							pageBpmScheme = bpmRectangle.fromRectangle(pageBpmScheme);
							pageBpmScheme.height = 827;
						}
					}
					else if (f.key == 'a5')
					{
						if (pageBpmScheme.width == 584)
						{
							pageBpmScheme = bpmRectangle.fromRectangle(pageBpmScheme);
							pageBpmScheme.width = 583;
						}
						else if (pageBpmScheme.height == 584)
						{
							pageBpmScheme = bpmRectangle.fromRectangle(pageBpmScheme);
							pageBpmScheme.height = 583;
						}
					}
					
					if (pageBpmScheme.width == f.format.width && pageBpmScheme.height == f.format.height)
					{
						paperSizeSelect.value = f.key;
					...
					else if (pageBpmScheme.width == f.format.height && pageBpmScheme.height == f.format.width)
					{
						paperSizeSelect.value = f.key;
			...
			if (!detected)
			{
				widthInput.value = pageBpmScheme.width / 100;
				heightInput.value = pageBpmScheme.height / 100;
				portraitCheckBox.setAttribute('checked', 'checked');
	...
	var currentPageBpmScheme = pageBpmScheme;
	...
	var update = function(evt, selectChanged)
	{
		...
		var newPageBpmScheme = new bpmRectangle(0, 0,
		...
		{
			newPageBpmScheme = new bpmRectangle(0, 0, newPageBpmScheme.height, newPageBpmScheme.width);
		}
		
		// Initial select of custom should not update page format to avoid update of combo
		if ((!selectChanged || !customSize) && (newPageBpmScheme.width != currentPageBpmScheme.width ||
			newPageBpmScheme.height != currentPageBpmScheme.height))
		{
			currentPageBpmScheme = newPageBpmScheme;
			
			// Updates page format and reloads format panel
			if (pageBpmSchemeListener != null)
			{
				pageBpmSchemeListener(currentPageBpmScheme);
			}
		}
	};

	bpmEvent.addListener(portraitSpan, 'click', function(evt)
	{
		portraitCheckBox.checked = true;
		update(evt);
		bpmEvent.consume(evt);
	...
	bpmEvent.addListener(landscapeSpan, 'click', function(evt)
	{
		landscapeCheckBox.checked = true;
		update(evt);
		bpmEvent.consume(evt);
	...
	bpmEvent.addListener(widthInput, 'blur', update);
	bpmEvent.addListener(widthInput, 'click', update);
	bpmEvent.addListener(heightInput, 'blur', update);
	bpmEvent.addListener(heightInput, 'click', update);
	bpmEvent.addListener(landscapeCheckBox, 'change', update);
	bpmEvent.addListener(portraitCheckBox, 'change', update);
	bpmEvent.addListener(paperSizeSelect, 'change', function(evt)
	{
 ...
 * 
 */
PageSetupBpmModal.getBpmSchemes = function()
{
	return [{key: 'letter', title: 'US-Letter (8,5" x 11")', format: bpmConstants.PAGE_FORMAT_LETTER_PORTRAIT},
	        {key: 'legal', title: 'US-Legal (8,5" x 14")', format: new bpmRectangle(0, 0, 850, 1400)},
	        {key: 'tabloid', title: 'US-Tabloid (279 mm x 432 mm)', format: new bpmRectangle(0, 0, 1100, 1700)},
	        {key: 'a0', title: 'A0 (841 mm x 1189 mm)', format: new bpmRectangle(0, 0, 3300, 4681)},
	        {key: 'a1', title: 'A1 (594 mm x 841 mm)', format: new bpmRectangle(0, 0, 2339, 3300)},
	        {key: 'a2', title: 'A2 (420 mm x 594 mm)', format: new bpmRectangle(0, 0, 1654, 2336)},
	        {key: 'a3', title: 'A3 (297 mm x 420 mm)', format: new bpmRectangle(0, 0, 1169, 1654)},
	        {key: 'a4', title: 'A4 (210 mm x 297 mm)', format: bpmConstants.PAGE_FORMAT_A4_PORTRAIT},
	        {key: 'a5', title: 'A5 (148 mm x 210 mm)', format: new bpmRectangle(0, 0, 583, 827)},
	        {key: 'a6', title: 'A6 (105 mm x 148 mm)', format: new bpmRectangle(0, 0, 413, 583)},
	        {key: 'a7', title: 'A7 (74 mm x 105 mm)', format: new bpmRectangle(0, 0, 291, 413)},
	        {key: 'custom', title: bpmResources.get('custom'), format: null}];
};
...
(function()
{
		...
		if (graph.container != null && !graph.transparentBackground)
		{
			if (graph.pageVisible)
			{
				...
				if (this.backgroundPageShape == null)
				{
					...
					while (firstChild != null && firstChild.nodeType != bpmConstants.NODETYPE_ELEMENT)
					{
					...
					if (firstChild != null)
					{
						...
						// also has known rendering issues inside bpmWindow but not using shadow is worse.
						this.backgroundPageShape.isShadow = !bpmCore.IS_QUIRKS;
						this.backgroundPageShape.dialect = bpmConstants.DIALECT_STRICTHTML;
						...
						this.backgroundPageShape.node.className = 'mainBackgroundPage';
						
						// Adds listener for double click handling on background
						bpmEvent.addListener(this.backgroundPageShape.node, 'dblclick',
						...
						bpmEvent.addGestureListeners(this.backgroundPageShape.node,
							...
							bpmUtils.bind(this, function(evt)
							{
								...
								if (graph.isMouseDown && !bpmEvent.isConsumed(evt))
								{
	...
	bpmGraphView.prototype.validateBackgroundStyles = function()
	{
		var graph = this.graph;
		var color = (graph.background == null || graph.background == bpmConstants.NONE) ? graph.defaultPageBackgroundColor : graph.background;
		...
		if (graph.isGridEnabled())
		{
			...
			position = -Math.round(phase - bpmUtils.mod(this.translate.x * this.scale - x0, phase)) + 'px ' +
				-Math.round(phase - bpmUtils.mod(this.translate.y * this.scale - y0, phase)) + 'px';
		}
		...
		if (graph.view.backgroundPageShape != null)
		{
			...
			graph.container.className = 'mainDiagramContainer geDiagramBackdrop';
			canvas.style.backgroundImage = 'none';
			canvas.style.backgroundColor = '';
		}
		else
		{
			graph.container.className = 'mainDiagramContainer';
	...
	bpmGraphView.prototype.createSvgGrid = function(color)
	{
		...
		var size = tmp2;
		var svg =  '<svg width="' + size + '" height="' + size + '" xmlns="' + bpmConstants.NS_SVG + '">' +
	...
	var bpmGraphPanGraph = bpmGraph.prototype.panGraph;
	bpmGraph.prototype.panGraph = function(dx, dy)
	{
		bpmGraphPanGraph.apply(this, arguments);
		...
		if (this.shiftPreview1 != null)
		{
			...
			var position = -Math.round(phase - bpmUtils.mod(this.view.translate.x * this.view.scale + dx, phase)) + 'px ' +
				-Math.round(phase - bpmUtils.mod(this.view.translate.y * this.view.scale + dy, phase)) + 'px';
			canvas.style.backgroundPosition = position;
		}
	};
	...
	bpmGraph.prototype.updatePageBreaks = function(visible, width, height)
	{
		...
		var bounds = new bpmRectangle(scale * tr.x, scale * tr.y, fmt.width * ps, fmt.height * ps);
			...
			if (breaks != null)
			{
				...
				for (var i = 0; i <= count; i++)
				{
					var pts = (breaks == this.horizontalPageBreaks) ?
						[new bpmPoint(Math.round(bounds2.x), Math.round(bounds2.y + (i + 1) * bounds.height)),
						 new bpmPoint(Math.round(right), Math.round(bounds2.y + (i + 1) * bounds.height))] :
						[new bpmPoint(Math.round(bounds2.x + (i + 1) * bounds.width), Math.round(bounds2.y)),
						 new bpmPoint(Math.round(bounds2.x + (i + 1) * bounds.width), Math.round(bottom))];
	...
	var bpmGraphHandlerShouldRemoveCellsFromParent = bpmGraphHandler.prototype.shouldRemoveCellsFromParent;
	bpmGraphHandler.prototype.shouldRemoveCellsFromParent = function(parent, cells, evt)
	{
		...
		return bpmGraphHandlerShouldRemoveCellsFromParent.apply(this, arguments);
	};
	...
	{
		return new bpmRectangleShape(bounds, '#ffffff', this.graph.defaultPageBorderColor);
	};
	...
	bpmGraphView.prototype.getBackgroundPageBounds = function()
	{
		...
		var fmt = this.graph.pageBpmScheme;
		...
		var bounds = new bpmRectangle(this.scale * (this.translate.x + x0 * pw), this.scale *
	...
	var graphPanGraph = bpmGraph.prototype.panGraph;
	bpmGraph.prototype.panGraph = function(dx, dy)
	{
		graphPanGraph.apply(this, arguments);
		
		if ((this.dialect != bpmConstants.DIALECT_SVG && this.view.backgroundPageShape != null) &&
			(!this.useScrollbarsForPanning || !bpmUtils.hasScrollbars(this.container)))
		{
	 ...
	 */
	var bpmPopupMenuAddItem = bpmPopupMenu.prototype.addItem;
	bpmPopupMenu.prototype.addItem = function(title, image, funct, parent, iconCls, enabled)
	{
		var result = bpmPopupMenuAddItem.apply(this, arguments);
		
		if (enabled != null && !enabled)
		{
			bpmEvent.addListener(result, 'mousedown', function(evt)
			{
				bpmEvent.consume(evt);
			});
		}
	...
	var graphHandlerGetInitialCellForEvent = bpmGraphHandler.prototype.getInitialCellForEvent;
	...
	var graphHandlerIsDelayedSelection = bpmGraphHandler.prototype.isDelayedSelection;
