...
function Toolbar(editorUi, container)
{
	...
	{
		if (this.editorUi.currentMenu != null && bpmEvent.getSource(evt) != this.editorUi.currentMenu.div)
		{
			this.hideMenu();
		}
	});
...
Toolbar.prototype.dropdownImageHtml = '<img border="0" style="position:absolute;right:4px;top:' +
	((!BpmUi.compactUi) ? 8 : 6) + 'px;" src="' + Toolbar.prototype.dropdownImage + '" valign="middle"/>';
...
Toolbar.prototype.init = function()
{
	...
	if (sw >= 700)
	{
		var formatMenu = this.addMenu('', bpmResources.get('view') + ' (' + bpmResources.get('panTooltip') + ')', true, 'viewPanels', null, true);
		this.addDropDownArrow(formatMenu, 'geSprite-formatpanel', 38, 50, -4, -3, 36, -8);
		this.addSeparator();
	}
	
	var viewMenu = this.addMenu('', bpmResources.get('zoom') + ' (Alt+Mousewheel)', true, 'viewZoom', null, true);
	...
	if (BpmUi.compactUi)
	{
		viewMenu.style.width = (bpmCore.IS_QUIRKS) ? '58px' : '50px';
	}
	else
	{
		viewMenu.style.width = (bpmCore.IS_QUIRKS) ? '62px' : '36px';
	}
	...
	if (sw >= 420)
	{
		...
		elts[0].setAttribute('title', bpmResources.get('zoomIn') + ' (' + this.editorUi.actions.get('zoomIn').shortcut + ')');
		elts[1].setAttribute('title', bpmResources.get('zoomOut') + ' (' + this.editorUi.actions.get('zoomOut').shortcut + ')');
	}
	
	// Updates the label if the scale changes
	this.updateZoom = bpmUtils.bind(this, function()
	{
	...
	this.editorUi.editor.graph.view.addListener(bpmEvent.EVENT_SCALE, this.updateZoom);
	this.editorUi.editor.addListener('resetGraphView', this.updateZoom);

	var elts = this.addItems(['-', 'undo', 'redo']);
	elts[1].setAttribute('title', bpmResources.get('undo') + ' (' + this.editorUi.actions.get('undo').shortcut + ')');
	elts[2].setAttribute('title', bpmResources.get('redo') + ' (' + this.editorUi.actions.get('redo').shortcut + ')');
	
	if (sw >= 320)
	{
		var elts = this.addItems(['-', 'delete']);
		elts[1].setAttribute('title', bpmResources.get('delete') + ' (' + this.editorUi.actions.get('delete').shortcut + ')');
	}
	...
	if (sw >= 400)
	{
		...
		if (sw >= 440)
		{
			this.edgeShapeMenu = this.addMenuFunction('', bpmResources.get('connection'), false, bpmUtils.bind(this, function(menu)
			{
				this.editorUi.menus.edgeStyleChange(menu, '', [bpmConstants.STYLE_SHAPE, 'width'], [null, null], 'geIcon geSprite geSprite-connection', null, true).setAttribute('title', bpmResources.get('line'));
				this.editorUi.menus.edgeStyleChange(menu, '', [bpmConstants.STYLE_SHAPE, 'width'], ['link', null], 'geIcon geSprite geSprite-linkedge', null, true).setAttribute('title', bpmResources.get('link'));
				this.editorUi.menus.edgeStyleChange(menu, '', [bpmConstants.STYLE_SHAPE, 'width'], ['flexArrow', null], 'geIcon geSprite geSprite-arrow', null, true).setAttribute('title', bpmResources.get('arrow'));
				this.editorUi.menus.edgeStyleChange(menu, '', [bpmConstants.STYLE_SHAPE, 'width'], ['arrow', null], 'geIcon geSprite geSprite-simplearrow', null, true).setAttribute('title', bpmResources.get('simpleArrow'));
			}));
	
			this.addDropDownArrow(this.edgeShapeMenu, 'geSprite-connection', 44, 50, 0, 0, 22, -4);
		}
	
		this.edgeStyleMenu = this.addMenuFunction('geSprite-orthogonal', bpmResources.get('waypoints'), false, bpmUtils.bind(this, function(menu)
		{
	...
	var insertMenu = this.addMenu('', bpmResources.get('insert') + ' (' + bpmResources.get('doubleClickTooltip') + ')', true, 'insert', null, true);
...
Toolbar.prototype.addDropDownArrow = function(menu, sprite, width, atlasWidth, left, top, atlasDelta, atlasLeft)
{
	...
	left = (BpmUi.compactUi) ? left : atlasLeft;
	...
	menu.innerHTML = '<div class="geSprite ' + sprite + '" style="margin-left:' + left + 'px;margin-top:' + top + 'px;"></div>' +
		this.dropdownImageHtml;
	menu.style.width = (bpmCore.IS_QUIRKS) ? atlasWidth + 'px' : (atlasWidth - atlasDelta) + 'px';
	
	if (bpmCore.IS_QUIRKS)
	{
		menu.style.height = (BpmUi.compactUi) ? '24px' : '26px';
	}
	...
	if (BpmUi.compactUi)
	{
		...
		menu.style.width = (bpmCore.IS_QUIRKS) ? width + 'px' : (width - 10) + 'px';
	}
};
...
Toolbar.prototype.setFontName = function(value)
{
	if (this.fontMenu != null)
	{
		this.fontMenu.innerHTML = '<div style="width:60px;overflow:hidden;display:inline-block;">' +
			bpmUtils.htmlEntities(value) + '</div>' + this.dropdownImageHtml;
	}
};
...
Toolbar.prototype.createTextToolbar = function()
{
	...
	var styleElt = this.addMenu('', bpmResources.get('style'), true, 'formatBlock');
	styleElt.style.position = 'relative';
	styleElt.style.whiteSpace = 'nowrap';
	styleElt.style.overflow = 'hidden';
	styleElt.innerHTML = bpmResources.get('style') + this.dropdownImageHtml;
	...
	this.fontMenu = this.addMenu('', bpmResources.get('fontFamily'), true, 'fontFamily');
	this.fontMenu.style.position = 'relative';
	this.fontMenu.style.whiteSpace = 'nowrap';
	this.fontMenu.style.overflow = 'hidden';
	this.fontMenu.style.width = (bpmCore.IS_QUIRKS) ? '80px' : '60px';
	...
	this.sizeMenu = this.addMenu(Menus.prototype.defaultFontSize, bpmResources.get('fontSize'), true, 'fontSize');
	this.sizeMenu.style.position = 'relative';
	this.sizeMenu.style.whiteSpace = 'nowrap';
	this.sizeMenu.style.overflow = 'hidden';
	this.sizeMenu.style.width = (bpmCore.IS_QUIRKS) ? '44px' : '24px';
	...
	elts[1].setAttribute('title', bpmResources.get('undo') + ' (' + this.editorUi.actions.get('undo').shortcut + ')');
	elts[2].setAttribute('title', bpmResources.get('redo') + ' (' + this.editorUi.actions.get('redo').shortcut + ')');
	elts[4].setAttribute('title', bpmResources.get('bold') + ' (' + this.editorUi.actions.get('bold').shortcut + ')');
	elts[5].setAttribute('title', bpmResources.get('italic') + ' (' + this.editorUi.actions.get('italic').shortcut + ')');
	elts[6].setAttribute('title', bpmResources.get('underline') + ' (' + this.editorUi.actions.get('underline').shortcut + ')');

	// KNOWN: Lost focus after click on submenu with text (not icon) in quirks and IE8. This is because the TD seems
	// to catch the focus on click in these browsers. NOTE: Workaround in bpmPopupMenu for icon items (without text).
	var alignMenu = this.addMenuFunction('', bpmResources.get('align'), false, bpmUtils.bind(this, function(menu)
	{
		elt = menu.addItem('', null, bpmUtils.bind(this, function(evt)
		{
			graph.cellEditor.alignText(bpmConstants.ALIGN_LEFT, evt);
		}), null, 'geIcon geSprite geSprite-left');
		elt.setAttribute('title', bpmResources.get('left'));

		elt = menu.addItem('', null, bpmUtils.bind(this, function(evt)
		{
			graph.cellEditor.alignText(bpmConstants.ALIGN_CENTER, evt);
		}), null, 'geIcon geSprite geSprite-center');
		elt.setAttribute('title', bpmResources.get('center'));

		elt = menu.addItem('', null, bpmUtils.bind(this, function(evt)
		{
			graph.cellEditor.alignText(bpmConstants.ALIGN_RIGHT, evt);
		}), null, 'geIcon geSprite geSprite-right');
		elt.setAttribute('title', bpmResources.get('right'));

		elt = menu.addItem('', null, bpmUtils.bind(this, function()
		{
			document.execCommand('justifyfull', false, null);
		}), null, 'geIcon geSprite geSprite-justifyfull');
		elt.setAttribute('title', bpmResources.get('justifyfull'));
		
		elt = menu.addItem('', null, bpmUtils.bind(this, function()
		{
			document.execCommand('insertorderedlist', false, null);
		}), null, 'geIcon geSprite geSprite-orderedlist');
		elt.setAttribute('title', bpmResources.get('numberedList'));
		
		elt = menu.addItem('', null, bpmUtils.bind(this, function()
		{
			document.execCommand('insertunorderedlist', false, null);
		}), null, 'geIcon geSprite geSprite-unorderedlist');
		elt.setAttribute('title', bpmResources.get('bulletedList'));
		
		elt = menu.addItem('', null, bpmUtils.bind(this, function()
		{
			document.execCommand('outdent', false, null);
		}), null, 'geIcon geSprite geSprite-outdent');
		elt.setAttribute('title', bpmResources.get('decreaseIndent'));
		
		elt = menu.addItem('', null, bpmUtils.bind(this, function()
		{
			document.execCommand('indent', false, null);
		}), null, 'geIcon geSprite geSprite-indent');
		elt.setAttribute('title', bpmResources.get('increaseIndent'));
	}));
	...
	alignMenu.style.width = (bpmCore.IS_QUIRKS) ? '50px' : '30px';
	...
	var formatMenu = this.addMenuFunction('', bpmResources.get('format'), false, bpmUtils.bind(this, function(menu)
	{
		elt = menu.addItem('', null, this.editorUi.actions.get('subscript').funct,
			null, 'geIcon geSprite geSprite-subscript');
		elt.setAttribute('title', bpmResources.get('subscript') + ' (' + BpmDraw.ctrlKey + '+,)');

		elt = menu.addItem('', null, this.editorUi.actions.get('superscript').funct,
			null, 'geIcon geSprite geSprite-superscript');
		elt.setAttribute('title', bpmResources.get('superscript') + ' (' + BpmDraw.ctrlKey + '+.)');
	...
	formatMenu.innerHTML = '<div class="geSprite geSprite-dots" style="margin-left:-2px;"></div>' +
		this.dropdownImageHtml;
	formatMenu.style.width = (bpmCore.IS_QUIRKS) ? '50px' : '30px';
	...
	this.addButton('geIcon geSprite geSprite-code', bpmResources.get('html'), function()
	{
	...
	// FIXME: Uses mainButton here and geLabel in main menu
	var insertMenu = this.addMenuFunction('', bpmResources.get('insert'), true, bpmUtils.bind(this, function(menu)
	{
		menu.addItem(bpmResources.get('insertLink'), null, bpmUtils.bind(this, function()
		{
			this.editorUi.actions.get('link').funct();
		}));
		
		menu.addItem(bpmResources.get('insertImage'), null, bpmUtils.bind(this, function()
		{
			this.editorUi.actions.get('image').funct();
		}));
		
		menu.addItem(bpmResources.get('insertHorizontalRule'), null, bpmUtils.bind(this, function()
		{
	...
	insertMenu.innerHTML = '<div class="geSprite geSprite-plus" style="margin-left:-4px;margin-top:-3px;"></div>' +
		this.dropdownImageHtml;
	insertMenu.style.width = (bpmCore.IS_QUIRKS) ? '36px' : '16px';
	...
	if (BpmUi.compactUi)
	{
		...
		insertMenu.style.width = (bpmCore.IS_QUIRKS) ? '50px' : '30px';
	}
	...
	// KNOWN: Lost focus after click on submenu with text (not icon) in quirks and IE8. This is because the TD seems
	// to catch the focus on click in these browsers. NOTE: Workaround in bpmPopupMenu for icon items (without text).
	var elt = this.addMenuFunction('geIcon geSprite geSprite-table', bpmResources.get('table'), false, bpmUtils.bind(this, function(menu)
	{
		...
		else
    	{
			...
			elt.setAttribute('title', bpmResources.get('insertColumnBefore'));
			...
			elt.setAttribute('title', bpmResources.get('insertColumnAfter'));

			elt = menu.addItem('Delete column', null, bpmUtils.bind(this, function()
			{
				if (cell != null)
				{
					try
			...
			elt.setAttribute('title', bpmResources.get('deleteColumn'));
			...
			elt.setAttribute('title', bpmResources.get('insertRowBefore'));
			...
			elt.setAttribute('title', bpmResources.get('insertRowAfter'));
			...
			elt.setAttribute('title', bpmResources.get('deleteRow'));
			...
			elt.setAttribute('title', bpmResources.get('borderColor'));
			...
			elt.setAttribute('title', bpmResources.get('spacing'));
	...
	elt.style.width = (bpmCore.IS_QUIRKS) ? '50px' : '30px';
...
Toolbar.prototype.addClickHandler = function(elt, funct)
{
	if (funct != null)
	{
		bpmEvent.addListener(elt, 'click', function(evt)
		{
			if (elt.enabled)
			{
				funct(evt);
	...
	if (funct != null)
	{
		...
		bpmEvent.addListener(elt, 'click', bpmUtils.bind(this, function(evt)
		{
			if (show && (elt.enabled == null || elt.enabled))
			{
				...
				menu = new bpmPopupMenu(funct);
				menu.div.className += ' mainToolbarMenu';
				...
				menu.addListener(bpmEvent.EVENT_HIDE, bpmUtils.bind(this, function()
				{
