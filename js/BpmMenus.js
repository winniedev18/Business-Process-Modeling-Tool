...
Menus.prototype.defaultMenuItems = ['file', 'edit', 'view', 'tools', 'lanes'];
...
Menus.prototype.init = function()
{
	...
	var isGraphEnabled = bpmUtils.bind(graph, graph.isEnabled);

	this.customFonts = [];
	this.customFontSizes = [];

	this.put('fontFamily', new Menu(bpmUtils.bind(this, function(menu, parent)
	{
		var addItem = bpmUtils.bind(this, function(fontname)
		{
			var tr = this.styleChange(menu, fontname, [bpmConstants.STYLE_FONTFAMILY], [fontname], null, parent, function()
			{
		...
		if (this.customFonts.length > 0)
		{
			...
			menu.addItem(bpmResources.get('reset'), null, bpmUtils.bind(this, function()
			{
				this.customFonts = [];
				this.editorUi.fireEvent(new bpmEventObject('customFontsChanged'));
			}), parent);
			
			menu.addSeparator(parent);
		}
		
		this.promptChange(menu, bpmResources.get('custom') + '...', '', bpmConstants.DEFAULT_FONTFAMILY, bpmConstants.STYLE_FONTFAMILY, parent, true, bpmUtils.bind(this, function(newValue)
		{
			if (bpmUtils.indexOf(this.customFonts, newValue) < 0)
			{
	...
	this.put('formatBlock', new Menu(bpmUtils.bind(this, function(menu, parent)
	{
		function addItem(label, tag)
		{
			return menu.addItem(label, null, bpmUtils.bind(this, function()
			{
		...
		addItem(bpmResources.get('normal'), 'p');
		
		addItem('', 'h1').firstChild.nextSibling.innerHTML = '<h1 style="margin:0px;">' + bpmResources.get('heading') + ' 1</h1>';
		addItem('', 'h2').firstChild.nextSibling.innerHTML = '<h2 style="margin:0px;">' + bpmResources.get('heading') + ' 2</h2>';
		addItem('', 'h3').firstChild.nextSibling.innerHTML = '<h3 style="margin:0px;">' + bpmResources.get('heading') + ' 3</h3>';
		addItem('', 'h4').firstChild.nextSibling.innerHTML = '<h4 style="margin:0px;">' + bpmResources.get('heading') + ' 4</h4>';
		addItem('', 'h5').firstChild.nextSibling.innerHTML = '<h5 style="margin:0px;">' + bpmResources.get('heading') + ' 5</h5>';
		addItem('', 'h6').firstChild.nextSibling.innerHTML = '<h6 style="margin:0px;">' + bpmResources.get('heading') + ' 6</h6>';
		
		addItem('', 'pre').firstChild.nextSibling.innerHTML = '<pre style="margin:0px;">' + bpmResources.get('formatted') + '</pre>';
		addItem('', 'blockquote').firstChild.nextSibling.innerHTML = '<blockquote style="margin-top:0px;margin-bottom:0px;">' + bpmResources.get('blockquote') + '</blockquote>';
	})));
	this.put('fontSize', new Menu(bpmUtils.bind(this, function(menu, parent)
	{
		var sizes = [6, 8, 9, 10, 11, 12, 14, 18, 24, 36, 48, 72];
		
		var addItem = bpmUtils.bind(this, function(fontsize)
		{
			this.styleChange(menu, fontsize, [bpmConstants.STYLE_FONTSIZE], [fontsize], null, parent, function()
			{
				if (graph.cellEditor.textarea != null)
				{
		...
		this.promptChange(menu, bpmResources.get('custom') + '...', '(pt)', '12', bpmConstants.STYLE_FONTSIZE, parent, true, bpmUtils.bind(this, function(newValue)
		{
			this.customFontSizes.push(newValue);
		}));
	})));
	this.put('direction', new Menu(bpmUtils.bind(this, function(menu, parent)
	{
		menu.addItem(bpmResources.get('flipH'), null, function() { graph.toggleCellStyles(bpmConstants.STYLE_FLIPH, false); }, parent);
		menu.addItem(bpmResources.get('flipV'), null, function() { graph.toggleCellStyles(bpmConstants.STYLE_FLIPV, false); }, parent);
		this.addMenuItems(menu, ['-', 'rotation'], parent);
	})));
	this.put('align', new Menu(bpmUtils.bind(this, function(menu, parent)
	{
		menu.addItem(bpmResources.get('leftAlign'), null, function() { graph.alignCells(bpmConstants.ALIGN_LEFT); }, parent);
		menu.addItem(bpmResources.get('center'), null, function() { graph.alignCells(bpmConstants.ALIGN_CENTER); }, parent);
		menu.addItem(bpmResources.get('rightAlign'), null, function() { graph.alignCells(bpmConstants.ALIGN_RIGHT); }, parent);
		menu.addSeparator(parent);
		menu.addItem(bpmResources.get('topAlign'), null, function() { graph.alignCells(bpmConstants.ALIGN_TOP); }, parent);
		menu.addItem(bpmResources.get('middle'), null, function() { graph.alignCells(bpmConstants.ALIGN_MIDDLE); }, parent);
		menu.addItem(bpmResources.get('bottomAlign'), null, function() { graph.alignCells(bpmConstants.ALIGN_BOTTOM); }, parent);
	})));
	this.put('distribute', new Menu(bpmUtils.bind(this, function(menu, parent)
	{
		menu.addItem(bpmResources.get('horizontal'), null, function() { graph.distributeCells(true); }, parent);
		menu.addItem(bpmResources.get('vertical'), null, function() { graph.distributeCells(false); }, parent);
	})));
	this.put('layout', new Menu(bpmUtils.bind(this, function(menu, parent)
	{
		var promptSpacing = bpmUtils.bind(this, function(defaultValue, fn)
		{
			var dlg = new FilenameBpmModal(this.editorUi, defaultValue, bpmResources.get('apply'), function(newValue)
			{
		...
		menu.addItem(bpmResources.get('horizontalFlow'), null, bpmUtils.bind(this, function()
		{
			var layout = new bpmHierarchicalLayout(graph, bpmConstants.DIRECTION_WEST);
		...
		menu.addItem(bpmResources.get('verticalFlow'), null, bpmUtils.bind(this, function()
		{
			var layout = new bpmHierarchicalLayout(graph, bpmConstants.DIRECTION_NORTH);
		...
		menu.addSeparator(parent);
		menu.addItem(bpmResources.get('horizontalTree'), null, bpmUtils.bind(this, function()
		{
			...
			if (tmp != null)
			{
				var layout = new bpmCompactTreeLayout(graph, true);
				layout.edgeRouting = false;
				layout.levelDistance = 30;
				
				promptSpacing(layout.levelDistance, bpmUtils.bind(this, function(newValue)
				{
		...
		menu.addItem(bpmResources.get('verticalTree'), null, bpmUtils.bind(this, function()
		{
			var tmp = graph.getSelectionCell();
			var roots = null;
			...
			if (tmp != null)
			{
				var layout = new bpmCompactTreeLayout(graph, false);
				layout.edgeRouting = false;
				layout.levelDistance = 30;
		...
		menu.addItem(bpmResources.get('radialTree'), null, bpmUtils.bind(this, function()
		{
			var tmp = graph.getSelectionCell();
			var roots = null;
			...
			if (tmp != null)
			{
				var layout = new bpmRadialTreeLayout(graph, false);
		...
		menu.addSeparator(parent);
		menu.addItem(bpmResources.get('organic'), null, bpmUtils.bind(this, function()
		{
			var layout = new bpmFastOrganicLayout(graph);
			
			promptSpacing(layout.forceConstant, bpmUtils.bind(this, function(newValue)
			{
		...
		menu.addItem(bpmResources.get('circle'), null, bpmUtils.bind(this, function()
		{
			var layout = new bpmCircleLayout(graph);
	...
	this.put('navigation', new Menu(bpmUtils.bind(this, function(menu, parent)
	{
		this.addMenuItems(menu, ['home', '-', 'exitGroup', 'enterGroup', '-', 'expand', 'collapse', '-', 'collapsible'], parent);
	})));
	this.put('arrange', new Menu(bpmUtils.bind(this, function(menu, parent)
	{
	...
	this.put('insert', new Menu(bpmUtils.bind(this, function(menu, parent)
	{
		this.addMenuItems(menu, ['insertLink', 'insertImage'], parent);
	})));
	this.put('view', new Menu(bpmUtils.bind(this, function(menu, parent)
	{
	...
	this.put('viewPanels', new Menu(bpmUtils.bind(this, function(menu, parent)
	{
	...
	this.put('viewZoom', new Menu(bpmUtils.bind(this, function(menu, parent)
	{
	...
	this.put('file', new Menu(bpmUtils.bind(this, function(menu, parent)
	{
		// this.addMenuItems(menu, ['new', 'open', '-', 'save', 'saveAs', '-', 'import', 'export', '-', 'pageSetup', 'print'], parent);
		var keys = ['new', 'open', '-', 'save', 'saveAs', '-', 'recent'];
		keys = keys.concat(this.workfiles);
		this.addMenuItems(menu, keys, parent);
	})));

	this.put('edit', new Menu(bpmUtils.bind(this, function(menu, parent)
	{
		// this.addMenuItems(menu, ['undo', 'redo', '-', 'cut', 'copy', 'paste', 'delete', '-', 'duplicate', '-',
		//                          'editData', 'editTooltip', 'editStyle', '-', 'edit', '-', 'editLink', 'openLink', '-',
		//                          'selectVertices', 'selectEdges', 'selectAll', 'selectNone', '-', 'lockUnlock']);
	})));
	this.put('extras', new Menu(bpmUtils.bind(this, function(menu, parent)
	{
		this.addMenuItems(menu, ['copyConnect', 'collapseExpand', '-', 'editDiagram']);
	})));
	this.put('help', new Menu(bpmUtils.bind(this, function(menu, parent)
	{
		this.addMenuItems(menu, ['help', '-', 'about']);
	})));

	this.put('tools', new Menu(bpmUtils.bind(this, function(menu, parent)
	{
		
	})));

	this.put('lanes', new Menu(bpmUtils.bind(this, function(menu, parent)
	{
...
Menus.prototype.addSubmenu = function(name, menu, parent, label)
{
	...
	if (entry != null)
	{
		...
		{
			var submenu = menu.addItem(label || bpmResources.get(name), null, null, parent, null, enabled);
...
Menus.prototype.addInsertTableItem = function(menu)
{
	...
	var elt2 = menu.addItem('', null, bpmUtils.bind(this, function(evt)
	{
		var td = graph.getParentByName(bpmEvent.getSource(evt), 'TD');
	...
	var quirksCellHtml = '<img src="' + bpmCore.imageBasePath + '/transparent.gif' + '" width="16" height="16"/>';
	...
	bpmEvent.addListener(picker, 'mouseover', function(e)
	{
		var td = graph.getParentByName(bpmEvent.getSource(e), 'TD');
...
Menus.prototype.edgeStyleChange = function(menu, label, keys, values, sprite, parent, reset)
{
	return menu.addItem(label, null, bpmUtils.bind(this, function()
	{
		...
		try
		{
			...
			this.editorUi.fireEvent(new bpmEventObject('styleChanged', 'keys', keys,
...
{
	return bpmUtils.bind(this, function(post)
	{
		var graph = this.editorUi.editor.graph;
		graph.stopEditing(false);
		...
		try
		{
			for (var i = 0; i < keys.length; i++)
			{
				...
				if (keys[i] == bpmConstants.STYLE_ALIGN)
				{
					graph.updateLabelElements(graph.getSelectionCells(), function(elt)
					{
			...
			this.editorUi.fireEvent(new bpmEventObject('styleChanged', 'keys', keys, 'values', values,
...
Menus.prototype.promptChange = function(menu, label, hint, defaultValue, key, parent, enabled, fn, sprite)
{
	return menu.addItem(label, null, bpmUtils.bind(this, function()
	{
		...
		var dlg = new FilenameBpmModal(this.editorUi, value, bpmResources.get('apply'), bpmUtils.bind(this, function(newValue)
		{
		...
		}), bpmResources.get('enterValue') + ((hint.length > 0) ? (' ' + hint) : ''));
		this.editorUi.showBpmModal(dlg.container, 300, 80, true, true);
		dlg.init();
...
Menus.prototype.pickColor = function(key, cmd, defaultValue)
{
	...
	if (cmd != null && graph.cellEditor.isContentEditing())
	{
		...
		var dlg = new ColorBpmModal(this.editorUi, defaultValue || '000000', bpmUtils.bind(this, function(color)
		{
			graph.cellEditor.restoreSelection(selState);
			document.execCommand(cmd, false, (color != bpmConstants.NONE) ? color : 'transparent');
		}), function()
		{
			graph.cellEditor.restoreSelection(selState);
		});
		this.editorUi.showBpmModal(dlg.container, 230, h, true, true);
		dlg.init();
	}
	else
	{
		if (this.colorBpmModal == null)
		{
			this.colorBpmModal = new ColorBpmModal(this.editorUi);
		}
	
		this.colorBpmModal.currentColorKey = key;
		var state = graph.getView().getState(graph.getSelectionCell());
		var color = 'none';
		...
		{
			color = 'ffffff';
			this.colorBpmModal.picker.fromString('ffffff');
			this.colorBpmModal.colorInput.value = 'none';
		}
		else
		{
			this.colorBpmModal.picker.fromString(color);
		}
	
		this.editorUi.showBpmModal(this.colorBpmModal.container, 230, h, true, true);
		this.colorBpmModal.init();
	}
};
...
Menus.prototype.toggleStyle = function(key, defaultValue)
{
	...
	this.editorUi.fireEvent(new bpmEventObject('styleChanged', 'keys', [key], 'values', [value],
			'cells', graph.getSelectionCells()));
};
...
Menus.prototype.addShortcut = function(item, action)
{
	if (action.shortcut != null)
	{
		...
		bpmUtils.write(span, action.shortcut);
		td.appendChild(span);
	}
};
...
Menus.prototype.createPopupMenu = function(menu, cell, evt)
{
	...
	if (!graph.isSelectionEmpty())
	{
		...
		if (state != null)
		{
			...
			if (graph.getModel().isEdge(cell) && bpmUtils.getValue(state.style, bpmConstants.STYLE_EDGE, null) != 'entityRelationEdgeStyle' &&
				bpmUtils.getValue(state.style, bpmConstants.STYLE_SHAPE, null) != 'arrow')
			{
				var handler = graph.selectionCellsHandler.getHandler(cell);
				var isWaypoint = false;
				
				if (handler instanceof bpmEdgeHandler && handler.bends != null && handler.bends.length > 2)
				{
					var index = handler.getHandleForEvent(graph.updateMouseEvent(new bpmMouseEvent(evt)));
				...
				menu.addSeparator();
				this.addMenuItem(menu, 'turn', null, evt, null, bpmResources.get('reverse'));
			...
			if (graph.getSelectionCount() == 1)
			{
				...
				if (graph.getModel().isVertex(cell) && bpmUtils.getValue(state.style, bpmConstants.STYLE_IMAGE, null) != null)
				{
					menu.addSeparator();
					this.addMenuItem(menu, 'image', null, evt).firstChild.nextSibling.innerHTML = bpmResources.get('editImage') + '...';
				}
			}
		}
	}
...
Menus.prototype.createMenubar = function(container)
{
	...
	{
		(bpmUtils.bind(this, function(menu)
		{
			var label = bpmResources.get(menus[i]);
			var elt = menubar.addMenu(label, bpmUtils.bind(this, function()
			{
...
Menus.prototype.addworkfile = function(itemName)
{
	var len = this.workfiles.length;
	while(len > 2)
	{
		this.workfiles.shift()
	    len = this.workfiles.length;
	}
	this.workfiles.push(itemName);
}
...
Menus.prototype.menuCreated = function(menu, elt, className)
{
	if (elt != null)
	{
		className = (className != null) ? className : 'elementItem';
		...
		menu.addListener('stateChanged', function()
		{
			...
			if (!menu.enabled)
			{
				elt.className = className + ' bpmDisabled';
...
Menubar.prototype.addMenu = function(label, funct, before)
{
	...
	elt.className = 'elementItem';
	bpmUtils.write(elt, label);
	...
	if (funct != null)
	{
		...
		var clickHandler = bpmUtils.bind(this, function(evt)
		{
			if (show && elt.enabled == null || elt.enabled)
			{
				...
				var menu = new bpmPopupMenu(funct);
				...
				var offset = bpmUtils.getOffset(elt);
		...
		bpmEvent.addListener(elt, 'mousemove', bpmUtils.bind(this, function(evt)
		{
...
// Menu inherits from bpmEventSource
bpmUtils.extend(Menu, bpmEventSource);
